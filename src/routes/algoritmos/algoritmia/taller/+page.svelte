<script lang="ts">
  ...
  async function loadServerAttempts() {
    ...
    try {
      ...
      const examGrades = all.filter((g: any) => g.subject === 'Algoritmos - Taller Algoritmia');
    ...
    for (const entry of queue) {
      ...
      try {
        const res = await gradesApi.submitMine({
          subject: 'Algoritmos - Taller Algoritmia',
          score: entry.score,
...
<svelte:head>
  <title>Taller de Algoritmia - Algoritmos</title>
</svelte:head>
    ...
    <div class="welcome-screen">
      <div class="welcome-card">
        ...
        {:else}
          ...
          <a href="/algoritmos" class="back-btn" style="text-decoration:none">Volver al Menú Principal</a>
        {/if}
      </div>
    </div>
