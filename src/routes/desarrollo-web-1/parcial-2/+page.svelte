<script lang="ts">
  import { selectRandomQuestions } from '$lib/data/parcial2';
  import { gradesApi, authApi, API_URL, scheduleApi } from '$lib/api';
  ...
  async function loadServerAttempts() {
    ...
    try {
      ...
      const examGrades = all.filter((g: any) => g.subject === 'Desarrollo Web 1 - Parcial 2');
    ...
    for (const entry of queue) {
      ...
      try {
        const res = await gradesApi.submitMine({
          subject: 'Desarrollo Web 1 - Parcial 2',
          score: entry.score,
...
<svelte:head>
  <title>Parcial 2 - Desarrollo Web 1</title>
</svelte:head>
  ...
  {#if !started && !finished}
    <div class="welcome-screen">
      <div class="welcome-card">
        ...
        <h1>Parcial 2 - Desarrollo Web 1</h1>
        ...
        <div class="attempts-section">
          ...
          <p class="attempts-info">
            ...
            {:else}
              Dispones de <strong>4 intentos</strong> en total: <strong>3 Simulacros</strong> (antes del examen) y <strong>1 Evaluación</strong> (durante el examen).
            {/if}
          </p>
          <div class="attempts-grid">
            <div class="attempt-card {isUnlimited || (slots.remaining > 0 && slots.used < 3) ? 'available' : 'used'}">
              <div class="attempt-number">{getAttemptLabel(1)}</div>
              <div class="attempt-type-badge prep">Simulacro</div>
              <div class="attempt-status">
                {#if isUnlimited}
                  <span class="ready-badge">Disponible</span>
                {:else if getAttemptCount() >= 1}
                  <span class="used-badge">✓ Utilizado</span>
                {:else if slots.remaining > 0 && slots.used < 3}
                  <span class="ready-badge">Disponible</span>
                {:else}
  ...
  {:else if finished}
    <div class="finish-screen">
      <div class="finish-card">
        ...
        {/if}
        <p class="finish-info">Has completado el Parcial 2 de Desarrollo Web 1.</p>
