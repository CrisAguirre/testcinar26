<script lang="ts">
  ...
  async function loadServerAttempts() {
    ...
    try {
      ...
      const examGrades = all.filter((g: any) => g.subject === 'Desarrollo Web 2 - Taller Algoritmia');
    ...
    for (const entry of queue) {
      ...
      try {
        const res = await gradesApi.submitMine({
          subject: 'Desarrollo Web 2 - Taller Algoritmia',
          score: entry.score,
...
<svelte:head>
  <title>Taller de Algoritmia - Desarrollo Web 2</title>
</svelte:head>
    ...
    <div class="welcome-screen">
      <div class="welcome-card">
        ...
        {:else}
          ...
          <a href="/desarrollo-web-2" class="back-btn" style="text-decoration:none">Volver al Menú Principal</a>
        {/if}
      </div>
    </div>
